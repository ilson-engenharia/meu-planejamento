...
Fonte: Planilha original do usuário (4b84b19a) — apenas correções de Português e Pontuação aplicadas.
"""
...
# ============================================================
# DADOS — 37 REGISTROS
# Fonte: Planilha original do usuário (4b84b19a — 21/05/2026)
# Apenas correções de Português e Pontuação aplicadas.
# Tupla: (elaboracao, projeto, empresa_resp, responsavel,
#         descricao, impacto, id_impact,
#         necessidade, observacao, conclusao_real)
# ============================================================
dados = [
    # 1 — CISTO / KROOM
    ...
    # 3 — CISTO / CRISTÁLIA  [corr: esta imepdindo→está impedindo, continidade→continuidade, Impedi→Impede, da Bases→das Bases]
    ...
    # 4 — CISTO / ABELV  [corr OBS: Pedente→Pendente]
    (date(2026,5,5), "CISTO", "ABELV",
     "Leobino (Eng. Civil Abelv)",
     "Chegada de Materiais para Fabricação da Tubulação do Trap",
     None, None,
     date(2026,5,7), "19/05: Pendente a entrega dos materiais", None),

    # 5 — CISTO / CRISTÁLIA  [corr desc: do Reatores→dos Reatores; imp: da das bases→das Bases, do Reatores→dos Reatores]
    ...
    # 8 — CISTO / ABELV  [corr desc: Lavadaor→Lavador; imp: Inicio→Início, da Piso→do Piso, Lavadaor→Lavador]
    ...
    # 10 — CISTO / ABELV  [corr imp: Lavadaor→Lavador, imapctando→impactando, da Piso→do Piso, Inicio→Início] — CONCLUÍDO COM ATRASO
    (date(2026,5,19), "CISTO", "ABELV",
     ...
     "(Início Previsto: 25/05/26 - Atividade do Cronograma: ID 466).",
     None,
     date(2026,5,19), None, date(2026,5,20)),

    # 11 — FARMO / KROOM  [corr desc: espaço duplo em "Tipo de  Fundação"; imp: novo impacto do usuário] — ID 431
    ...
    # 12 — FARMO / CRISTÁLIA  [corr imp: Inicio→Início]
    ...
    # 13 — FARMO / MGC  [corr imp: Subscontratações→Subcontratações] — CONCLUÍDO COM ATRASO
    ...
    # 14 — FARMO / ABELV  [corr desc: Lovador→Lavador]
    ...
    # 16 — FARMO / KROOM  [corr desc: informa→informar, do que→em que]
    ...
    # 17 — CISTO / KROOM  [corr desc: resistencia→resistência, contruido→construída; obs: alterantiva→alternativa, metódo→método; imp: da das→das, do Reatores→dos Reatores] — CONCLUÍDO
    ...
    # 19 — FARMO / KROOM  [corr imp: Imapcto→Impacto] — CONCLUÍDO
    (date(2026,5,19), "FARMO", "KROOM",
     "Rodrigo (Téc. Planej. da Kroom) / Leobino (Eng. Civil Abelv)",
     "A Kroom precisa enviar os Pedidos de Faturamento Direto",
     "Impacto na Medição",
     None,
     date(2026,5,19), None, date(2026,5,19)),

    # 20 — CISTO / KROOM  — CONCLUÍDO
    (date(2026,5,19), "CISTO", "KROOM",
     ...
     None,
     date(2026,5,20), None, date(2026,5,20)),

    # 21 — CISTO / ABELV  — CONCLUÍDO
    (date(2026,5,19), "CISTO", "ABELV",
     "Leobino (Eng. Civil Abelv)",
    ...
    # 22 — CISTO / ABELV  [corr resp: EriK→Erik]
    (date(2026,5,19), "CISTO", "ABELV",
    ...
    # 24 — FARMO / ABELV  [corr imp: aditoria→auditoria] — ALERTA
    (date(2026,5,19), "FARMO", "ABELV",
     "Diego (Gerente de Projetos da Kroom) / Leobino (Eng. Civil Abelv)",
    ...
    # 25 — CISTO / ABELV  [corr resp: Engenheiro→Engenheira; desc: não cabe→não cabem; imp: do Reatores→dos Reatores, Térmico→Término] — ID 650
    (date(2026,5,19), "CISTO", "ABELV",
     "Yohanna (Engenheira Mecânica Abelv)",
    ...
    # 26 — CISTO / CRISTÁLIA  [corr desc: adquiro→adquirido] — ID 9
    (date(2026,5,19), "CISTO", "CRISTÁLIA",
     "Yohanna (Engenheira Mecânica Abelv)",
    ...
    # 27 — CISTO / CRISTÁLIA  [corr desc: Locaçao→Locação, do Reatores→dos Reatores, estão→está, cooredenada→coordenada, do Ralos→dos Ralos] — ID 59
    (date(2026,5,19), "CISTO", "CRISTÁLIA",
     "Yohanna (Engenheira Mecânica Abelv)",
    ...
    # 28 — CISTO / CRISTÁLIA  [corr desc: Tinta Asfaltica→Tinta Asfáltica, nos Dique→nos Diques] — CONCLUÍDO
    (date(2026,5,20), "CISTO", "CRISTÁLIA",
     "Leobino (Eng. Civil Abelv)",
    ...
    (date(2026,5,20), "CISTO", "ABELV",
     ...
     None, None,
     date(2026,5,25), None, None),

    # 30 — CISTO / KROOM  [corr imp: a nivel→a nível]
    (date(2026,5,20), "CISTO", "KROOM",
     "Diego (Gerente de Projetos da Kroom) / Leobino (Eng. Civil Abelv)",
    ...
    # 31 — CISTO / ABELV  [corr imp: Impacto;→Impacto:] — ALERTA — ID 169
    (date(2026,5,21), "CISTO", "ABELV",
    ...
    # 32 — CISTO / ABELV  [corr desc: esta→está; imp: Pintura Asfaltica→Pintura Asfáltica] — ALERTA — ID 279
    (date(2026,5,21), "CISTO", "ABELV",
     "Leobino (Eng. Civil Abelv)",
    ...
    # 33 — CISTO / ABELV  [corr imp: locado→localizada] — ALERTA — ID 22
    (date(2026,5,21), "CISTO", "ABELV",
     "Leobino (Eng. Civil Abelv)",
    ...
    # 34 — CISTO / ABELV  [corr desc: Citostátcio→Citostático, solcitado→solicitado, "Krrom ."→"Krrom."] — ID 284
    (date(2026,5,21), "CISTO", "ABELV",
    ...
    # 35 — CISTO / ABELV  [corr desc: Mecâncio→Mecânico, todos equipamentos→todos os equipamentos; imp: Conteção→Contenção] — ALERTA — ID 276
    (date(2026,5,21), "CISTO", "ABELV",
     "Leobino (Eng. Civil Abelv)",
    ...
    # 36 — CISTO / ABELV  [corr desc: Inical→Inicial, Qunatitativo→Quantitativo]
    (date(2026,5,21), "CISTO", "ABELV",
     "Yohanna (Engenheira Mecânica Abelv)",
     "Levantamento Inicial do Quantitativo de Tintas para Obra.",
     None, None,
     date(2026,5,29), None, None),

    # 37 — CISTO / CRISTÁLIA  [corr desc: espaço final removido]
    (date(2026,5,21), "CISTO", "CRISTÁLIA",
     "Leobino (Eng. Civil Abelv)",
...
headers = [
    ...
    "DESCRIÇÃO", "IMPACTO", "ID IMPACT",
    "NECESSIDADE", "OBSERVAÇÃO", "CONCLUSÃO REAL", "DIAS ATRASADO", "STATUS"
]
N_COLS      = 13
COL_ELAB   = 3
COL_ID_IMP = 8
...
col_widths = [5, 10, 13, 14, 35, 50, 50, 12, 13, 35, 14, 10, 22]
...
for idx, row in enumerate(dados, 1):
    r = idx + 1
    elab, proj, emp, resp, desc, imp, id_imp, nec, obs, conc = row

    status   = calc_status(nec, conc)
    dias_atr = calc_dias(nec, conc)
    proj_bg  = C_CISTO_BG if proj == "CISTO" else C_FARMO_BG

    values = [idx, proj, elab, emp, resp, desc, imp, id_imp, nec, obs, conc, dias_atr, status]
    ...
    for col, val in enumerate(values, 1):
        ...
        if col in (COL_ELAB, COL_NEC, COL_CONC) and isinstance(val, date):
...
c = wd.cell(row=1, column=2, value="DASHBOARD — RESTRIÇÕES | Cristália 25098 + 25103 | 21/05/2026")
c.fill = pf(C_HEADER_BG); c.font = ft(bold=True, color="FFFFFF", size=13)
c.alignment = al(h="center"); wd.row_dimensions[1].height = 30

status_list = [calc_status(d[7], d[9]) for d in dados]
...
resp_atr = Counter(d[3] for d in dados if calc_status(d[7], d[9]) == "ATRASADO")
...
alertas = [(d[3], d[4], d[7]) for d in dados if calc_status(d[7], d[9]) == "ALERTA"]
wd.merge_cells("E13:F13")
dash_cell(wd, 13, 5, "⚠ ALERTA — VENCE HOJE (21/05)",
...
# SALVAR
# ============================================================
output_path = "/home/user/meu-planejamento/Restricoes_Consolidadas_Cristalia_21052026.xlsx"
wb.save(output_path)
