#!/usr/bin/env python3
"""PUSH LIST REV2 — ASU Jundiaí (26001) | OTZ Engenharia × Messer Gases for Life"""
...
# ── Gauge SVG (shared for full and mini) ──────────────────────────────────────

def gauge_svg(pct, done_cnt, total_cnt, uid, mini=False):
    if mini:
        W, H, cx, cy, R = 155, 118, 77, 72, 52
        lbl_sz, pct_sz, sub_sz = 8, 18, 7.5
        hub_r, hub_r2 = 6, 3
        tick_out, tick_in = 7, 4
    else:
        W, H, cx, cy, R = 260, 195, 130, 118, 88
        lbl_sz, pct_sz, sub_sz = 9, 26, 8.5
        hub_r, hub_r2 = 9, 4
        tick_out, tick_in = 10, 6
  ...
  <linearGradient id="zg{uid}" gradientUnits="userSpaceOnUse"
    x1="{polar(210,R)[0]:.0f}" y1="{polar(210,R)[1]:.0f}"
    x2="{polar(-30,R)[0]:.0f}" y2="{polar(-30,R)[1]:.0f}">
    ...
    # Track layers
    s.append(f'<path d="{arc_d(R,START,end_ang)}" fill="none" stroke="#1A3A60" '
             f'stroke-width="{16 if not mini else 12}" stroke-linecap="round"/>')
    s.append(f'<path d="{arc_d(R,START,end_ang)}" fill="none" stroke="url(#zg{uid})" '
             f'stroke-width="{16 if not mini else 12}" stroke-linecap="round" opacity="0.12"/>')

    # Progress arc
    if pct > 0.1:
        s.append(f'<path d="{arc_d(R,START,prog_ang)}" fill="none" stroke="{color}" '
                 f'stroke-width="{14 if not mini else 10}" stroke-linecap="round" '
                 f'filter="url(#gl{uid})" opacity="0.95"/>')

    # Ticks
    for tp in ([0,25,50,75,100] if not mini else [0,50,100]):
        ta = START - tp/100*SPAN
        xi, yi = polar(ta, R - tick_in)
        xo, yo = polar(ta, R + tick_out)
        ...
        if not mini:
            ...
            s.append(f'<text x="{xl:.1f}" y="{yl:.1f}" text-anchor="middle" '
                     f'dominant-baseline="middle" fill="#4A90D9" font-size="{lbl_sz}" '
                     f'font-family="Arial,sans-serif">{tp}%</text>')

    # Needle (triangle)
    nx, ny = polar(prog_ang, R - 8)
    b1x, b1y = polar(prog_ang + 90, 5)
    b2x, b2y = polar(prog_ang - 90, 5)
    s.append(f'<polygon points="{cx:.1f},{cy:.1f} {b1x:.1f},{b1y:.1f} '
             f'{nx:.1f},{ny:.1f} {b2x:.1f},{b2y:.1f}" '
             f'fill="white" filter="url(#gl{uid})" opacity="0.88"/>')

    # Hub
    s.append(f'<circle cx="{cx}" cy="{cy}" r="{hub_r}" fill="#0D1F3C" '
             f'stroke="#00BCD4" stroke-width="2"/>')
    s.append(f'<circle cx="{cx}" cy="{cy}" r="{hub_r2}" fill="#00BCD4"/>')

    # Text
    pct_str = f"{pct:.0f}%"
    s.append(f'<text x="{cx}" y="{cy+28}" text-anchor="middle" fill="white" '
             f'font-size="{pct_sz}" font-weight="bold" font-family="Arial,sans-serif">{pct_str}</text>')
    s.append(f'<text x="{cx}" y="{cy+40}" text-anchor="middle" fill="#00BCD4" '
             f'font-size="{sub_sz}" font-family="Arial,sans-serif">AVANÇO FÍSICO</text>')
    if not mini:
        s.append(f'<text x="{cx}" y="{cy+52}" text-anchor="middle" fill="#4A90D9" '
...
# ── Discipline chart SVG ──────────────────────────────────────────────────────

def discipline_chart_svg(disc_stats):
    """Horizontal bar chart per discipline, sorted by total desc."""
    ...
    STA_W = 110
    PAD   = 20
    W     = LBL_W + BAR_W + STA_W + PAD * 2
    H     = len(disc_stats) * ROW_H + PAD * 2
    max_t = max(d[3] for d in disc_stats) or 1
    ...
    for i, (name, color, done, total) in enumerate(disc_stats):
        y    = PAD + i * ROW_H
        bw   = int(BAR_W * total / max_t)
        dw   = int(BAR_W * done / max_t)
        pct  = done / total * 100 if total else 0
        ymid = y + ROW_H // 2

        # Label
        s.append(f'<text x="{PAD}" y="{ymid}" dominant-baseline="middle" '
                 f'fill="#C8D8F0" font-size="12.5" font-family="Arial,sans-serif">{name}</text>')

        # Background bar
        bx = PAD + LBL_W
        s.append(f'<rect x="{bx}" y="{ymid-10}" width="{bw}" height="20" '
                 f'rx="5" fill="{color}22" stroke="{color}40" stroke-width="1"/>')

        # Progress bar
        if dw > 0:
            s.append(f'<rect x="{bx}" y="{ymid-10}" width="{dw}" height="20" '
                     f'rx="5" fill="{color}" opacity="0.85"/>')

        # Count + pct
        sx = bx + BAR_W + 12
        s.append(f'<text x="{sx}" y="{ymid}" dominant-baseline="middle" '
                 f'fill="{color}" font-size="12" font-family="Arial,sans-serif" '
                 f'font-weight="600">{done}/{total}</text>')
        s.append(f'<text x="{sx+52}" y="{ymid}" dominant-baseline="middle" '
                 f'fill="#4A90D9" font-size="11.5" font-family="Arial,sans-serif">'
                 f'{pct:.0f}%</text>')

        # Separator line
        if i < len(disc_stats) - 1:
            s.append(f'<line x1="{PAD}" y1="{y+ROW_H}" x2="{W-PAD}" y2="{y+ROW_H}" '
...
def read_excel():
    ...
    for row in ws.iter_rows(min_row=4, values_only=True):
        ...
        card_data[key]["atividades"].append({
            ...
            "status":status,"previsao":prev_,"conclusao":conc})
    return card_data
...
def build_cards(card_data):
    ...
    for lnum in sorted(from_local.keys()):
        ...
        # Count of photo-cards remaining in Excel for this local
        ...
        for fidx, fstr in entries:
            ...
            if fstr == "S/F":
                # Platô Central: has photos in meta, create one card per photo
                if fotos:
                    foto_total = len(fotos)
                    for fi, fname in enumerate(fotos):
                        gid += 1
                        print(f"  Encoding {fname} ...", end=" ", flush=True)
                        b64 = encode_photo(fname)
                        print("ok" if b64 else "FAIL")
                        atividades, dp, tp = _build_atividades(data["atividades"])
                        pct = dp/tp*100 if tp else 0
                        cards.append(_card(gid, data, meta, lnum,
                                           fi+1, foto_total, b64, False,
                                           atividades, dp, tp, pct))
                else:
                    gid += 1
                    atividades, dp, tp = _build_atividades(data["atividades"])
                    pct = dp/tp*100 if tp else 0
                ...
                pct = dp/tp*100 if tp else 0
                cards.append(_card(gid, data, meta, lnum,
                                   seq, photo_rem, b64, False,
                                   atividades, dp, tp, pct))

    return cards

def _build_atividades(raw):
    atividades = []
    dp = tp = 0
    for i, a in enumerate(raw):
        ...
        atividades.append({**a, "num": i+1})
    return atividades, dp, tp

def _card(gid, data, meta, lnum, seq, total, b64, sem_foto,
          atividades, dp, tp, pct):
    return {
        "id": gid, "area": data["area"], "nome": data["nome"],
        "local_num": lnum, "foto_seq": seq, "foto_total": total,
        "foto_b64": b64, "sem_foto": sem_foto,
        "gps": meta.get("gps",""), "horario": meta.get("horario",""),
        "data_lev": meta.get("data",""), "endereco": meta.get("endereco",""),
        "atividades": atividades, "done_peso": dp,
        "total_peso": tp, "pct": pct,
    }
...
def compute_kpis(cards):
    ...
    disc_map = defaultdict(lambda: [0, 0])  # [done, total]
    for c in cards:
    ...
    disc_stats = [(d, disc_color(d), v[0], v[1])
                  for d, v in disc_map.items()]
    ...
    return {
        "tot_cards": tot_cards, "tot_atv": tot_atv,
        "done_atv": done_atv, "pend_atv": pend_atv,
        "pct": pct, "dias_aberto": DIAS_ABERTO,
        "velocidade": vel, "previsao": prev,
...
def html_card(card):
    ...
    if card["sem_foto"]:
        foto_h = '''<div class="photo-placeholder">
  <div style="font-size:30px;opacity:.4">📷</div>
  <div style="font-size:11px">Foto não disponível</div>
</div>'''
        badge_h = ""
    else:
        foto_h  = (f'<img class="card-photo" '
                   f'src="data:image/jpeg;base64,{card["foto_b64"]}" '
                   f'alt="Local {card["local_num"]} foto {card["foto_seq"]}">')
        badge_h = (f'<div class="photo-badge">Foto {card["foto_seq"]}'
...
<div class="push-card" id="card-{card["id"]}">
  <div class="card-hbar" style="background:linear-gradient(135deg,hsl({lhue},55%,12%),hsl({lhue},40%,18%))">
    ...
    <span class="cstats">
      <span class="sb done">{done_cnt}✓</span>
      <span class="sb pend">{pend_cnt}◌</span>
    </span>
  </div>
  ...
  <div class="card-info-row">
    <div class="card-meta">
      <div class="mrow"><span class="mi">⊕</span>{card["gps"]}</div>
      <div class="mrow"><span class="mi">◷</span>{card["horario"]} — {card["data_lev"]}</div>
      <div class="mrow addr"><span class="mi">⊞</span>{card["endereco"]}</div>
    </div>
    <div class="card-mini-gauge">
      {mini}
    </div>
  </div>
...
def generate_html(cards, kpis, otz_b64):
  ...
  <text x="0" y="64" font-family="Arial,sans-serif" font-size="15"
    ...
    # Global gauge
    global_gauge = gauge_svg(k["pct"], k["done_atv"], k["tot_atv"], "global", mini=False)
    ...
    kpi_boxes = "".join([
        ...
        kbox("⏱", "Dias em Aberto",   str(k["dias_aberto"]),
             "#B0BEC5", f'desde {LEVAN_DATE.strftime("%d/%m/%Y")}'),
    ])
    if k["velocidade"]:
...
<title>PUSH LIST — DAENG | ASU Jundiaí (26001) — REV2</title>
<style>
...
.header-refbar{{background:#06101E;border-top:1px solid #1A3A60;
                padding:8px 20px;font-size:10.5px;color:#4A90D9;
                display:flex;flex-wrap:wrap;gap:8px;align-items:center}}
.rpill{{background:#0D1F3C;border:1px solid #1A3A60;border-radius:4px;
...
.kpi-row{{display:flex;gap:12px;align-items:stretch;flex-wrap:wrap}}
.kpi-boxes{{display:flex;flex-wrap:wrap;gap:10px;flex:1}}
.kpi-box{{background:#0D1F3C;border:1px solid #1A3A60;border-radius:8px;
          padding:10px 14px;min-width:110px;flex:1;text-align:center;
...
.sec-title{{font-size:11px;color:#4A90D9;letter-spacing:2px;text-transform:uppercase;
            margin-bottom:12px}}

/* ── Discipline chart section ── */
.disc-section{{background:#07111E;border-bottom:1px solid #1A3A60;padding:16px 20px}}
.disc-inner{{background:#0D1F3C;border:1px solid #1A3A60;border-radius:10px;
             padding:16px 20px}}

/* ── Curva-S placeholder ── */
.curvas-section{{background:#06101E;border-bottom:1px solid #1A3A60;padding:14px 20px}}
.curvas-inner{{background:#0D1F3C;border:1px dashed #1A3A60;border-radius:10px;
               padding:20px;text-align:center;color:#2A4A70}}
.curvas-icon{{font-size:32px;opacity:.5;margin-bottom:8px}}
.curvas-title{{font-size:13px;font-weight:600;color:#4A90D9;margin-bottom:4px}}
.curvas-sub{{font-size:11px;color:#2A5080}}
...
.cards-grid{{display:grid;grid-template-columns:repeat(auto-fill,minmax(680px,1fr));gap:16px}}
...
.card-hbar{{display:flex;align-items:center;gap:8px;padding:7px 14px;
            border-bottom:1px solid #1A3A6080}}
.cid-badge{{background:#00BCD4;color:#0A1628;font-size:10.5px;font-weight:700;
            border-radius:4px;padding:2px 7px;white-space:nowrap}}
.cloc-badge{{background:#1A3A60;color:#7BAFD4;font-size:10.5px;
             border-radius:4px;padding:2px 7px;white-space:nowrap}}
.cnome{{flex:1;font-size:13px;font-weight:600;color:#C8D8F0;
        white-space:nowrap;overflow:hidden;text-overflow:ellipsis}}
.cpct{{font-size:15px;font-weight:700;white-space:nowrap;margin-right:4px}}
.cstats{{display:flex;gap:5px}}
.sb{{font-size:10px;border-radius:3px;padding:1px 6px;font-weight:600}}
...
/* ── Photo full width ── */
.card-photo-wrap{{position:relative;height:260px;background:#050F1A;overflow:hidden;
                  border-bottom:1px solid #1A3A60}}
.card-photo{{width:100%;height:100%;object-fit:cover;display:block}}
.photo-placeholder{{width:100%;height:100%;display:flex;flex-direction:column;
                    align-items:center;justify-content:center;gap:8px;color:#2A4A70}}
.photo-badge{{position:absolute;bottom:8px;right:10px;background:#00000090;
              color:#00BCD4;font-size:10px;border-radius:4px;padding:2px 7px;
              backdrop-filter:blur(4px)}}
.area-overlay{{position:absolute;top:8px;left:10px;background:#00000080;
               color:#C8D8F0;font-size:11px;font-weight:600;border-radius:4px;
               padding:3px 9px;backdrop-filter:blur(4px);max-width:70%}}

/* ── Meta + mini gauge row ── */
.card-info-row{{display:flex;align-items:center;border-bottom:1px solid #1A3A60;
                background:#081524}}
.card-meta{{flex:1;padding:10px 14px;font-size:10.5px;color:#7BAFD4;
            display:flex;flex-direction:column;gap:5px}}
.mrow{{display:flex;align-items:flex-start;gap:5px}}
.mi{{color:#00BCD4;min-width:14px;flex-shrink:0}}
.addr{{font-size:10px;color:#4A90D9;line-height:1.3}}
.card-mini-gauge{{padding:6px 10px 6px 0;flex-shrink:0}}

/* ── Activity table ── */
.card-activities{{overflow-x:auto}}
.atv-table{{width:100%;border-collapse:collapse;font-size:12px}}
.atv-table thead tr{{background:#06101E;border-bottom:2px solid #00BCD430}}
.atv-table th{{padding:6px 8px;color:#4A90D9;font-size:10.5px;font-weight:600;
...
.atv-num{{padding:7px 8px;color:#4A90D9;font-weight:600;text-align:center;white-space:nowrap}}
.atv-disc{{padding:7px 8px;color:#C8D8F0;font-size:11.5px;white-space:nowrap}}
.ddot{{display:inline-block;width:8px;height:8px;border-radius:50%;
       margin-right:5px;vertical-align:middle}}
.atv-desc{{padding:7px 8px;color:#B0C8E8;line-height:1.35}}
.atv-peso{{padding:7px 6px;text-align:center}}
.pbadge{{display:inline-block;width:22px;height:22px;line-height:22px;text-align:center;
         border-radius:50%;font-weight:700;font-size:11px;border:2px solid;color:#C8D8F0}}
...
<header class="site-header">
  ...
  <div class="header-refbar">
    ...
    <span class="rpill" style="color:#00BCD4;font-weight:600">REV2 — {today_str}</span>
  </div>
</header>
...
<section class="disc-section">
  <div class="sec-title">&#9670; Atividades por Disciplina — Avanço Físico</div>
  <div class="disc-inner">{disc_chart}</div>
</section>

<!-- Curva-S placeholder -->
<section class="curvas-section">
  <div class="curvas-inner">
    <div class="curvas-icon">📈</div>
    <div class="curvas-title">Curva-S de Avanço — Previsto × Realizado</div>
...
<section class="cards-section">
  <div class="sec-header">&#9670; Locais de Serviço &nbsp;·&nbsp; {len(cards)} cards &nbsp;·&nbsp; {k["tot_atv"]} atividades</div>
...
<footer class="site-footer">
  ...
  <div class="footer-rev">REV2 &nbsp;·&nbsp; {today_str}</div>
</footer>
...
def generate_xlsx(cards):
    ...
    c.value = (f"Ref: CLM-216  |  Contratada: Andrade e Rocha (DAENG)  |  "
               ...
               f"Levantamento: {LEVAN_DATE.strftime('%d/%m/%Y')}  |  REV2 — {TODAY.strftime('%d/%m/%Y')}  |  "
    ...
    headers = ["#Card","Área","Local Nº","Nome do Local","Foto","# Ativ.",
               ...
               "Previsão","Conclusão","Avanço (%)"]
    for col, h in enumerate(headers, 1):
        c = ws.cell(row=3, column=col, value=h)
        sc(c, bold=True, bg="0D2540", fg="00BCD4", sz=10, ha="center")
        c.border = brd_m
    ...
    row_num = 4
    for card in cards:
        n = len(card["atividades"])
        r0 = row_num
        for a in card["atividades"]:
            bg = "0D1A30" if row_num % 2 == 0 else "081020"
            vals = [card["id"], card["area"], card["local_num"], card["nome"],
                    card["foto_seq"] if card["foto_seq"] > 0 else "S/F",
                    a["num"], a["disciplina"], a["descricao"],
                    a["peso"], a["status"], a["previsao"] or "",
                    a["conclusao"] or "", f'{card["pct"]:.1f}%']
            for col, v in enumerate(vals, 1):
                ...
                c.border = brd
            ws.row_dimensions[row_num].height = 30 if len(str(a["descricao"]))>70 else 16
            row_num += 1
        if n > 1:
...
def main():
    print("="*60)
    print("PUSH LIST REV2 — ASU Jundiaí (26001)")
    print("="*60)

    print("\n[1] Reading Excel …")
    card_data = read_excel()
    print(f"    {len(card_data)} card keys")
    ...
    print(f"    Avanço físico: {kpis['pct']:.1f}%  |  Dias em aberto: {kpis['dias_aberto']}")

    print("\n[4] OTZ logo …")
    otz_b64 = load_otz_logo()
    print(f"    {'OK' if otz_b64 else 'not found'}")
