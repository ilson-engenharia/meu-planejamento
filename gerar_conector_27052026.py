...
# -*- coding: utf-8 -*-
"""
Restrições CONECTOR — 27/05/2026
46 itens | DISCIPLINA em formato [CIVIL], [MEC/TUB], etc. | 2 abas: Restrições + Dashboard
"""
import openpyxl
...
dados = [
    ...
    ("[CIVIL]", date(2026,5,11),"CISTO","KROOM",
     ...
     "25/05: Não foi mobilizada a equipe da Krrom (no momento 3 colaboradores), pois a Krrom não recebeu o sinal.",
     date(2026,5,11)),
    ...
    ("[MEC/TUB]", date(2026,5,5),"CISTO","ABELV",
     ...
     None, None, date(2026,5,7),
     "19/05: Pendente a entrega dos materiais.", None),
    ...
    ("[CIVIL]", date(2026,5,19),"FARMO","ABELV",
     ...
     None, date(2026,5,19), None, date(2026,5,21)),
    ...
    ("[CIVIL]", date(2026,5,19),"CISTO","KROOM",
     "Diego (Gerente de Projetos da Kroom) / Leobino (Eng. Civil Abelv)",
     "A Krrom precisa apresentar a Metodologia de Corte que será aplicada para avaliar a resistência "
    ...
    ("[DOC][SUP]", date(2026,5,19),"CISTO","ABELV",
     "Erik Massola (Comprador Abelv) / Tiago Matos (Gestor de Projetos Abelv)",
     ...
     "(Início Previsto: 21/05 - Atividade do Cronograma (Fabricação dos Dutos): ID 561).",
     561, date(2026,5,20), None, None),

    # ── ITEM 23 ──
    ("[SUP]", date(2026,5,19),"CISTO","ABELV",
     "Edimar Cunha (Supervisor de Suprimentos Abelv) / Tiago Matos (Gestor de Projetos Abelv)",
    ...
    ("[SMS]", date(2026,5,20),"CISTO","KROOM",
     ...
     None, date(2026,5,20),
     "26/05: Chegou o Pluviômetro no site.",
     date(2026,5,25)),
    ...
    ("[CIVIL]", date(2026,5,21),"CISTO","ABELV",
     ...
     279, date(2026,5,21),
     "26/05: Leobino, na Reunião de Bom Dia, informou que já elaborou a CT.",
     date(2026,5,25)),
    ...
    ("[CIVIL]", date(2026,5,21),"CISTO","ABELV",
     ...
     22, date(2026,5,21),
     "22/05: A GEOCOM realizou o Ensaio de Esclerometria.",
     date(2026,5,22)),
    ...
    ("[CIVIL]", date(2026,5,21),"CISTO","ABELV",
     ...
     "Impacto: Retrabalho nos Diques de Contenção do Piso Técnico, pois já foi iniciada essa atividade. "
    ...
    ("[CIVIL]", date(2026,5,21),"CISTO","CRISTÁLIA",
     ...
     "Impacto: Atraso na aplicação da Tinta Externa, porém sabendo que no dia 20/05, o Projeto Civil de "
     "Construção dos Diques não está de acordo com o apresentado pela Mecânica para a contenção dos "
     "equipamentos, já caracterizando retrabalho.",
     None, date(2026,5,22), None, None),

    # ── ITEM 38 ──
    ("[CIVIL]", date(2026,5,22),"FARMO","ABELV",
     "Leobino (Eng. Civil Abelv)",
     "Elaboração de Consulta Técnica (CT) para identificar a posição e trajetória da Linha de Incêndio "
     "encontrada na escavação, pois existe a possibilidade de topar com nova Rede de Drenagem Pluvial ou "
     "com a posição atual do Prédio que será construído. E saber qual será a tratativa de correção estrutural "
     "da mesma, tendo em vista que ela apresenta pontos de corrosão (diversos pontos), aparente baixa espessura "
     "e vazamento em vários pontos.",
     "Impacto: Possibilidade de atraso no início da Fundação ou da Linha de Drenagem, caso haja demora no "
     "retorno do cliente. (Início Previsto: 21/08 - Fundações - Atividade do Cronograma: ID 431; "
     "e Início Previsto: 22/12 - Drenagem Pluvial - Rota e PVs - Atividade do Cronograma: ID 381).",
     "381;431", date(2026,5,25),
     "25/05: Leobino informou que o Edson da Cristália já possuía o Projeto da Linha de Incêndio Aéreo "
     "e o forneceria até o dia 26/05/26.",
     None),

    # ── ITEM 39 ──
    ("[MEC/TUB]", date(2026,5,22),"FARMO","ABELV",
     "Uehara (Engenheiro Mecânico do HVAC Abelv)",
     "Elaboração de Consulta Técnica (CT) para saber qual tratativa a ser executada, tendo em vista que "
     "os Lavadores de Gases apresentam 1 Caixa de Ar que se comunica aos 2 Equipamentos do Farmoquímico. "
     "Para fazer a primeira manobra de instalar o primeiro e possibilitar a desativação e remoção do segundo "
     "do estado inicial, precisamos antes condicionar o primeiro removido com uma Caixa de Ar.",
     "Impacto: Caso não haja disponibilidade de 1 Caixa de Ar para a realização da primeira mudança do "
     "Lavador de Gases para a nova posição, poderá atrasar essa operação "
     "(Início Previsto: 14/07 - Atividade do Cronograma (Montagem do Lavador de Gases 01 (Reserva)): ID 262).",
     262, date(2026,5,25), None, None),

    # ── ITEM 40 ──
    ("[CIVIL]", date(2026,5,25),"CISTO","CRISTÁLIA",
     "Edson (Cristália) / Yohanna (Engenheira Mecânica Abelv)",
     "A Cristália deve enviar os Projetos dos Equipamentos (Banhos Térmicos dos Reatores: BT-008, BT-009 e BT-010), "
     "para que a Abelv possa verificar se os mesmos cabem dentro do Novo Dique Técnico. "
     "Essa informação foi solicitada por e-mail.",
     "Impacto: Caso atrase, e haja novamente a necessidade de mudança no Projeto do Dique de Contenção do "
     "Piso Técnico, teremos que novamente retrabalhar na confecção do Dique, cujo já teve retrabalho por "
     "necessidade dimensional dos equipamentos "
     "(Início Previsto: 21/05 - Atividade do Cronograma (Assentamento de Blocos de Alvenaria): ID 276).",
     276, date(2026,5,28), None, None),

    # ── ITEM 41 ──
    ("[DOC]", date(2026,5,25),"FARMO","CRISTÁLIA",
     "Edson (Cristália) / Ilson (Planejamento Abelv)",
     "Comunicar à Cristália que os Elevadores (EL-001 e EL-002) são de Fornecimento e Instalação da Cristália, conforme o Contrato.",
     "Impacto: Instalação dos elevadores (EL-001 e EL-002).",
     None, date(2026,5,25),
     "25/05: Já informado Lucas e Edson da Cristália via celular, dia 25/05/26.",
     date(2026,5,25)),

    # ── ITEM 42 ──
    ("[DOC][SUP]", date(2026,5,26),"CISTO","ABELV",
     "Celso (Gestor de Projetos Abelv)",
     "Solicitar à equipe da empresa contratada Açoplast (Estrutura Metálica) o preenchimento e envio para "
     "a Abelv da Planilha Nominal com os Dados dos Colaboradores que realmente irão ao site. "
     "O objetivo é realizar a avaliação social da Cristália.",
     "Impacto: Necessidade de contratação de outros colaboradores de forma emergencial para adentrar o site, "
     "consequentemente atraso no início da Fabricação dos Dutos de HVAC "
     "(Início Previsto: 21/05 - Atividade do Cronograma (Fabricação dos Dutos): ID 561).",
     561, date(2026,5,26), None, None),

    # ── ITEM 43 ──
    ("[MEC/TUB]", date(2026,5,26),"CISTO","ABELV",
     "Yohanna (Engenheira Mecânica Abelv)",
     "Adequação do Projeto da Linha de AG que conecta com a Linha TRAP (Isométrico).",
     "Impacto: Atraso na fabricação da Linha de Água Gelada, já em atraso pois até o dia 26/05/26 não "
     "chegaram as conexões para iniciar a Fabricação "
     "(Início Previsto: 25/05 - Atividade do Cronograma (Pré-Fabricação de Tubulação / AG): ID 261).",
     261, date(2026,5,27), None, None),

    # ── ITEM 44 ──
    ("[MEC/TUB]", date(2026,5,27),"FARMO","ABELV",
     "Uehara (Engenheiro Mecânico do HVAC Abelv)",
     "Elaboração do layout de posicionamento do lavador de gases existente do Farmoquímico.",
     "Impacto: Esta atividade afeta a subcontratada MGC na elaboração do projeto do radier do lavador de "
     "gases que receberá os equipamentos (Período previsto: 05/05 a 15/05/26 — Atividade do Cronograma: "
     "Elaboração do Projeto do Radier dos Lavadores de Gases, ID 15). Como consequência, haverá um atraso "
     "posterior na construção do radier do lavador de gases existente "
     "(Início previsto: 18/06 — Atividade do Cronograma: ID 251).",
     "15;251", date(2026,5,27), None, None),

    # ── ITEM 45 ──
    ("[MEC/TUB][SUP]", date(2026,5,27),"CISTO","ABELV",
     "Yohanna (Engenheira Mecânica Abelv)",
     "Elaboração de MC (Mapa de Cotação) para a compra dos materiais faltantes do suporte de tubulação, "
     "devido à incompatibilidade da LPU com o projeto recebido pela Cristália.",
     "Impacto: Atraso na fabricação dos suportes "
     "(Início previsto: 20/05 até o Término previsto: 16/06/26 — "
     "Atividade do Cronograma: Fabricação dos Suportes, ID 254).",
     254, date(2026,5,29), None, None),

    # ── ITEM 46 ──
    ("[CIVIL]", date(2026,5,27),"FARMO","KROOM",
     "Diego (Gerente de Projetos da Kroom) / Leobino (Eng. Civil Abelv)",
     "Finalização da escavação na região da construção da Sala Elétrica, para identificar interferências no local da fundação.",
     "Impacto: Atraso na remoção de eventuais interferências, devido à necessidade de avaliação pelo cliente, "
     "com consequente impacto no início dos serviços civis da Sala Elétrica e da base do radier do lavador "
     "de gases existente (Início previsto: 18/06 — Atividade do Cronograma: ID 251).",
     251, date(2026,5,27), None, None),
]
...
c=wd.cell(row=1,column=2,value="DASHBOARD — RESTRIÇÕES CONECTOR | Cristália 25098 + 25103 | 27/05/2026")
...
alertas=[(d[4],d[5],d[8]) for d in dados if calc_status(d[8],d[10])=="ALERTA"]
wd.merge_cells("E13:F13")
dc(wd,13,5,f"⚠ ALERTA — VENCE HOJE (27/05)",bg=C_AL_BG,bold=True,color=C_AL_FG,h="center")
